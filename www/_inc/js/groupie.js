...
function chatTimeout() {
  if(CHAT_TIMEOUT_COUNT > 1) {
    CHAT_TIMEOUT_COUNT = CHAT_TIMEOUT_COUNT - 1;
  } else {
    hideChat();
    CHAT_TIMEOUT_COUNT=0;
  }
...
var Groupie = {
	...
	var chat = $('#chat').get(0);
	CHAT_TIMEOUT_COUNT=CHAT_TIMEOUT_COUNT+1;
        setTimeout("chatTimeout()", 10000);
        chat.scrollTop = chat.scrollHeight;
//        $.get("http://10.20.30.51/cgi-bin/unblank", function(data){});
        return true;
    },
...
function autoConnect() {
  Groupie.room = 'loungecontrol@conference.jabber.metalab.at'
  Groupie.nickname = 'lounge'
  $(document).trigger('connect', {
    jid: 'lounge@jabber.metalab.at',
    password: 'dontfuckwithme'
  });
}
